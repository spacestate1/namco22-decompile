...
{
    memset(st, 0, sizeof *st);
    if (!live_cg) { live_cg = malloc(0x20000); if (!live_cg) return 0; }
    ...
    { const uint8_t *pp = renderer3d_planar_palette();
      st->pal = pp ? pp : g_sys.palette_ram; }
    st->owns_pal = 0;
    for (int i = 0; i < 8; i++)
        st->attr[i] = (uint16_t)((g_sys.tilemapattr[i*2] << 8) |
                                  g_sys.tilemapattr[i*2 + 1]);
    st->valid = 1;
    return 1;
}
