...
static uint32_t point_read(c71_t *d, uint32_t a)
{
    ...
    if (a >= C71_PTRAM_BASE && a < C71_PTRAM_BASE + C71_PTRAM_WORDS)
        return sx24(d->ptram[a - C71_PTRAM_BASE]);
    return 0xFFFFFFFF;
}
...
static void point_write(c71_t *d, uint32_t a, uint32_t v)
{
    ...
    if (a >= C71_PTRAM_BASE && a < C71_PTRAM_BASE + C71_PTRAM_WORDS)
        d->ptram[a - C71_PTRAM_BASE] = v & 0xFFFFFF;
}
...
static uint16_t port_in(c71_t *d, int pa)
{
    ...
    case 2: pdp_run(d); d->bioz = 0; return 0;      /* PDP begin, BIO ready */
...
{
    /* deliver a pended timer interrupt at the instruction boundary */
    if (d->tint_pend && !d->intm && (d->imr & 8)) {
        if (!push(d, d->pc)) return false;
        d->pc = 0x0018; d->intm = 1; d->tint_pend = 0;
    }
    int pc = d->pc;
    d->cur_pc = pc;
    d->steps++;
