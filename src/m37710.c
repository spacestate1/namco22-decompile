/* M37710 core -- see include/m37710.h for the board wiring and why this
 * exists rather than another model of the driver's output.
 *
...
static void sfr_w(m37710_t *c, uint32_t a, uint8_t v);
...
static inline uint8_t  fetch8 (m37710_t *c) { uint8_t v = rd8(c, ((uint32_t)c->pg << 16) | c->pc); c->pc++; return v; }
...
{
    int retired = 0;
    uint64_t until = c->cycles + (uint64_t)cycles;

    while (c->cycles < until && !c->unimpl_hit) {
        m37710_service(c);
        if (c->stopped) { c->cycles = until; break; }

        uint32_t op_pc = ((uint32_t)c->pg << 16) | c->pc;
        if (c->pchist_on) {
            c->pchist[c->pchist_n & 63] = c->pc; c->pchist_n++;
        }
        { extern int g_mcu_pc; static int shown;
          if (g_mcu_pc >= 0 && c->pc == (uint16_t)g_mcu_pc && shown < 4000) {
              shown++;
              fprintf(stderr, "[PC %04X] A=%04X B=%04X X=%04X Y=%04X DPR=%04X DT=%02X ps=%04X\n",
                      c->pc, c->a, c->b, c->x, c->y, c->dpr, c->dt, c->ps);
          } }
        /* The 0x42 page runs the SAME handlers with the accumulator accessors
         * pointed at B, so dispatch is on the base opcode; only the coverage
         * census distinguishes the two pages. */
        uint16_t op = fetch8(c);
        c->use_b = false;
        if (op == 0x42) { c->use_b = true; op = fetch8(c); }
        c->op_count[c->use_b ? (0x100 | op) : op]++;
        c->cycles += 1;
        retired++;
        ...
        default:
            /* STOP, do not skip. Report it so the probe can queue the work. */
            c->unimpl_hit = true;
            c->unimpl_op  = (uint16_t)(c->use_b ? (0x100 | op) : op);
            c->unimpl_pc  = op_pc;
            retired--;
            break;
        }
