...
/* The earliest cycle at which anything could interrupt the program: a timer
 * underflow or an A-D conversion. (Pins are the board's, see mcu_sound.c.) A
 * readable routine in src/snd/snd_engine.c runs as one block, so it may only
 * do so when this lies beyond its worst-case cost -- then no interrupt could
 * have landed between its instructions in the original either. */
uint64_t m37710_next_event(const m37710_t *c)
{
    uint64_t t = UINT64_MAX;
    for (int n = 0; n < 8; n++)
        if ((c->t_on & (1u << n)) && c->t_next[n] < t) t = c->t_next[n];
    if (c->ad_due && c->ad_due < t) t = c->ad_due;
    return t;
}
...
/* PROPCYCL_SNDTJITTER=<cycles>, TEST ONLY: delay every timer arm by that many
 * cycles -- Timer A0's phase once, and each Timer B0 one-shot. Asks whether the
 * driver's output ORDER depends on when its own interrupts land. */
static long g_tjitter = -1;
static void timer_arm(m37710_t *c, int n)
{
    if (g_tjitter < 0) { const char *e = getenv("PROPCYCL_SNDTJITTER"); g_tjitter = e ? atol(e) : 0; }
    ...
    c->t_on |= (uint8_t)(1u << n);
    c->t_next[n] = c->cycles + (uint64_t)(reload + 1) * TSCALE[(mode >> 6) & 3] + (uint64_t)g_tjitter;
}
