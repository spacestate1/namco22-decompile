/*
 * ss22_env.c -- the TRACE ORACLE's environment (dev builds only; the shipped game has no such thing).
 *
 ...
 * <tag>_ENV=<file> in the game's trace build; without RR_TRACE this file is empty stubs.
 */
#include <stdio.h>
#include <stdlib.h>
#include "ss22_game.h"
...
static void apply(const env_t *e)
{
    switch (e->kind) {
    case 'E': {
        if (e->addr >= 0x400000u && e->addr < 0x400020u) {     /* the keycus: MAME's random answer, returned by the read that follows */
            ss22_hw_keycus_force(e->value);
            break;
        }
    ...
    case 'I':
        for (int line = 0; line < 4; line++)                /* the syscon line that owns this level */
            if ((g_ss22.syscon[line] & 7) == e->level && (g_hw.irq_enabled & (1u << line))) { g_hw.irq_state |= 1u << line; break; }
...
int ss22_env_init(const char *path)
{
    if (!path) return 0;
    FILE *f = fopen(path, "r");
    if (!f) { fprintf(stderr, "[%s] cannot open %s_ENV %s\n", g_ss22_game->tag, g_ss22_game->tag, path); return 0; }
    ...
    fclose(f);
    fprintf(stderr, "[%s] trace oracle: %zu main-flow + %zu handler environment records from %s\n", g_ss22_game->tag, nrec, nerec, path);
    rr_trace_hook = hook;
    active = 1;
    atexit(ss22_env_report);                                   /* the trace build exits from inside the trace hook */
    return 1;
}
int ss22_env_active(void) { return active; }
#else
void ss22_env_report(void) {}
int ss22_env_init(const char *path) { (void)path; return 0; }
int ss22_env_active(void) { return 0; }
#endif
