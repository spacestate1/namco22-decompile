/*
 * tw_snd.c -- the Super System 22 SOUND MCU BOARD as the M37710 sees it (namcos22.cpp mcu_program):
 ...
 *   0x004000-0x00BFFF  shared RAM with the 68K  -> g_tw.shared, byte lanes ^1
 *   0x00C000-0x00FFFF  tw1data.8k[0xC000..]     the S22-BIOS
 *   0x200000-0x27FFFF  tw1data.8k               the sound program + sequence data
 ...
 * d5/d6 strobe P5's output latch); P5 reads INPUTS; P6 reads 0. A-D channels 0/2/3 are the wheel and
 * the two pedals. The board drives IRQ2 at scanline 240 and IRQ0 at scanline 480 of a 525-line frame
...
/* how the sound program's instructions get executed: gen/tw_snd_driver.c (the translation) in the game,
 * tools/sndoracle/sound_oracle.c (fetch/decode) in tw_oracle */
...
#define SLICES     16u                   /* tw_main's slices per frame */
#define VLINES     525u
#define CHIP_DIV   192u                  /* MCU cycles per C352 output frame */

static m37710_t  cpu;
static uint8_t  *rom;                    /* tw1data.8k, 0x80000 */
...
static int64_t   pin_shift;              /* MCU cycles added to every pin time (TW_SNDPHASE: where in its frame the reset release falls) */
static double    want_phase = 97;        /* the scanline of its frame at which the MCU leaves reset (TW_SNDPHASE overrides, < 0 = wherever the 68K gets there) */
static uint64_t  chip_done;              /* C352 frames generated so far */

static uint16_t  pressed;                /* TW_IN_* bits currently held */
static uint16_t  adc[4] = { 0x200, 0, 0, 0 };
...
static uint8_t bus_r(void *u, uint32_t a)
{
    ...
    if (a >= 0x004000 && a < 0x00C000) return g_tw.shared[(a - 0x4000) ^ 1];
...
{
    (void)u;
    if (a >= 0x004000 && a < 0x00C000) { g_tw.shared[(a - 0x4000) ^ 1] = v; return; }
    if (a >= 0x002000 && a < 0x003000) {
        unsigned off = a - 0x2000;
        c352sh[off] = v;
        if (c352log)
...
static uint8_t port_r(void *u, unsigned reg, uint8_t latch)
{
    ...
    case 0x0B: {                                                                          /* mcu_port5_r */
        const uint16_t inputs = (uint16_t)(0xFFFFu & ~pressed);                           /* IP_ACTIVE_LOW, idle high */
...
bool tw_snd_init(const char *dir)
{
    rom = calloc(1, 0x80000);
    wave = calloc(1, 0x400000);
    if (!rom || !wave || !slurp(dir, "tw1data.8k", rom, 0x80000)) return false;
    chip_ok = slurp(dir, "tw1wavea.2l", wave, 0x400000);
    if (chip_ok) { c352_init(&chip, wave, 0x400000); c352_reset(&chip); }
    m37710_init(&cpu, bus_r, bus_w, NULL);
    cpu.port_r = port_r; cpu.port_w = port_w;
    snd_executor_init();
    const char *d = getenv("TW_AUDIODUMP");
    if (d && (dump = fopen(d, "wb"))) { wav_header(dump, 0); fprintf(stderr, "[SND] dumping the front pair to %s\n", d); }
    const char *l = getenv("TW_C352LOG");
    ...
    if (getenv("TW_MCUTRAP")) cpu.pchist_on = 1;
    if (getenv("TW_SNDPHASE")) want_phase = atof(getenv("TW_SNDPHASE"));
    ready = true;
    tw_snd_inputs(0, 0x200, 0, 0);                        /* wheel centred, pedals up, nothing pressed */
    return true;
}
...
void tw_snd_set_run(bool run)
{
    if (!ready || run == running) return;
    if (getenv("TW_SNDLOG")) fprintf(stderr, "[SND] syscon 0x16 <- %d after %llu board cycles\n", run, (unsigned long long)grid);
     ...
     * times (its first register write, at 3.34166 s, is line ~97 of frame 200); TW_SNDPHASE=<line> overrides */
    pin_shift = 0;
    if (want_phase >= 0) {
...
bool tw_snd_faulted(void) { return faulted; }
void tw_snd_set_output(bool on) { out_live = on; }
void tw_snd_inputs(uint16_t p, unsigned wheel, unsigned fwd, unsigned bwd)
{
    pressed = p;
    adc[0] = (uint16_t)(wheel & 0x3FF); adc[2] = (uint16_t)(fwd & 0x3FF); adc[3] = (uint16_t)(bwd & 0x3FF);
    cpu.analog[0] = adc[0]; cpu.analog[2] = adc[2]; cpu.analog[3] = adc[3];
}
uint16_t tw_snd_outputs(void) { return outputs; }
...
void tw_snd_debug(char *buf, int n)
{
    ...
    snprintf(buf, n, "snd: run %d fault %d pc %02X%04X irqs %llu cycles %llu ps %04X out %04X in %04X ad %03X/%03X/%03X",
             ...
             cpu.ps, outputs, (unsigned)(0xFFFFu & ~pressed), adc[0], adc[2], adc[3]);
}
