...
# Build Dirt Dash.
#
#   ./build.sh /path/to/dirtdash.zip                         first time
#   ./build.sh                                               later: just rebuild
#
# The ROM set is MAME's dirtdash.zip (World, DT2 Ver.A). If you don't pass a
...
"No ROMs yet. Run:   ./build.sh /path/to/dirtdash.zip
    (MAME's Dirt Dash set. It is not included: you need your own copy.)"
fi

# The game's program is big generated C (gen/dd_lifted.c, 26 MB); the first build takes several minutes.
...
cmake --build build --target dd -j"$(nproc)"

printf '\nDone! From the top folder, start the game with:   ./launch.sh dirt\n'
printf '  5 = coin (two per game)   Enter = view / confirm   arrows = steer   Esc = menu\n\n'
