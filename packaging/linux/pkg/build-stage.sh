#!/bin/sh
# Build both games from an exported public tree and lay out the installed files.
#   build-stage.sh SRC ROMDIR STAGE
# ROMDIR holds propcycl.zip, raverace.zip and namcoc74.zip: each game's sound
...
install -m755 -s "$SRC/build/propcycl" "$SRC/raverace/build/rr" "$STAGE/usr/lib/namco22/"
install -m755 "$PKG/namco22-launch" "$STAGE/usr/lib/namco22/"
install -m755 "$PKG/propcycle" "$PKG/raveracer" "$STAGE/usr/bin/"
install -m644 "$PKG/propcycle.desktop" "$PKG/raveracer.desktop" "$STAGE/usr/share/applications/"
install -m644 "$PKG/propcycle.png" "$PKG/raveracer.png" "$STAGE/usr/share/icons/hicolor/256x256/apps/"
